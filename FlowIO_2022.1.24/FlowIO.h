...
private:
	...
	uint32_t _startTimes[9]={0,0,0,0,0,0,0,0,0}; //holds the start time for each valve & pump actuated.
		//The indexing matches the hardwareState table: {p1,p2,p3,p4,p5,inlet,outlet,pump1,pump2}
		//0 means the valve is off.
		//-1 means the valve is ON and has been set to a holding current value.
		//Any other value means the valve is currently drawing maximum current.
