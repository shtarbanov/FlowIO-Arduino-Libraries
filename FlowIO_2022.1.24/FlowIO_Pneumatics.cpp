...
void FlowIO::startInflation(uint8_t ports, uint8_t pwmValue) {
    stopAction(ports);
    if (_config == GENERAL || _config == INFLATION_PARALLEL || _config == INFLATION_SERIES || _config == REGULATED_PRESSURE ) {
        if (ports << 3 != 0) { //If a user sends the "ports" argument with all 5 LSBs as 0, then don't do anything.
            ...
            setPorts(ports);
            if (_config == REGULATED_PRESSURE) {
                setRegulator(pwmValue);
            } else {
...
void FlowIO::startInflationHalfCapacity(uint8_t ports,
                                        uint8_t pwmValue) { //Only applicable for Inflation_Series and Inflation_Parallel configurations.
    stopAction(ports);
    if (_config == INFLATION_PARALLEL || _config == INFLATION_SERIES || _config == REGULATED_PRESSURE) {
        if (ports << 3 != 0) { //If a user sends the "ports" argument with all 5 LSBs as 0, then don't do anything.
            openInletValve();
            setPorts(ports);
            if (_config == REGULATED_PRESSURE) {
...
void FlowIO::startVacuum(uint8_t ports, uint8_t pwmValue) {
    stopAction(ports);
    if (_config == GENERAL || _config == VACUUM_PARALLEL || _config == VACUUM_SERIES || _config == REGULATED_VACUUM) {
        if (ports << 3 != 0) { //If a user sends the "ports" argument with all 5 LSBs as 0, then don't do anything.
            openOutletValve();
            setPorts(ports);
            if (_config == REGULATED_VACUUM) {
                setRegulator(pwmValue);
            } else {
...
void FlowIO::startVacuumHalfCapacity(uint8_t ports,
                                     uint8_t pwmValue) { //Only applicable for Vacuum_Series and Vacuum_Parallel configurations.
    stopAction(ports);
    if (_config == VACUUM_SERIES || _config == VACUUM_PARALLEL || _config == REGULATED_VACUUM) {
        if (ports << 3 != 0) {
            openOutletValve();
            setPorts(ports);
            if (_config == REGULATED_VACUUM) {
...
void FlowIO::startRelease(uint8_t ports) {
    ...
    if (ports << 3 != 0) {
        ...
        if (_config == INFLATION_PARALLEL || _config == INFLATION_SERIES || _config == REGULATED_PRESSURE) {
            openOutletValve();
        } else if (_config == VACUUM_SERIES || _config == VACUUM_PARALLEL || _config == REGULATED_VACUUM) {
            openInletValve();
...
void FlowIO::stopAction(uint8_t ports) {
    ...
    if (_config == REGULATED_PRESSURE || _config == REGULATED_VACUUM) {
        setRegulator(0);
    } else {
        stopPump(1);
        stopPump(2);
