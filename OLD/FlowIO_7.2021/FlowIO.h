...
private:
	...
	//Another reason we want the ports to start from 0 is because the bits we want the last 5 bits of the "ports"
  ...
  Unit _pressureUnit=PSI;
	...
	//Status indicators
	int _valveOpenedAt[7]={0,0,0,0,0,0,0}; //holds the start time for each valve actuated.
		//The indexing matches the hardwareState table: {p1,p2,p3,p4,p5,inlet,outlet}
		//0 means the valve is off.
		//0 also means the valve has been set to a holding current value.
		//Anything other than 0 means the vale is currently drawing maximum current.
