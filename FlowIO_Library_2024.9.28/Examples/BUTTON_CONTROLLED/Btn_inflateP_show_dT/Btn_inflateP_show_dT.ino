...
#define PORTS 0b00000010 //port 2.
#define PMAX 23
#define PUNIT PSI
...
void loop() {
  ...
  if(activeFlag){
    inflationTime = flowio.inflateP(PORTS,PMAX,PUNIT); //returns 0 when complete, 1 when running, time when stopping.
    if(inflationTime>1){
      Serial.print("T(ms) = ");
      Serial.println(inflationTime);
      Serial.print("P = ");
      Serial.println(flowio.getPressure(PUNIT));
    }
  }
}
